...
def get_activation(w, row, b):
  activation = 0
  for i in range(len(w)):
    activation += w[i]*row[i]
  activation += b
  return activation

def update_weights(w, a, x, y, b):
  for i in range (len(w)):
    w[i] += x[i] * y
...
def train_perceptron(data):
  ...
  for i in range (0, MAX_ITERS):
    ...
    for j in range (len(data)):
      activation = get_activation(w,data[j][0],b)
      if data[j][1]*activation <=0:
        converged = False
        update_weights(w, activation, data[j][0], data[j][1], b)
        b += data[j][1]
    if converged:
      return (w,b)
  return (w,b)
...
def predict_perceptron(model, x):
  ...
  for j in range (len(w)):
    activation = get_activation(model[0], x, model[1])
  return activation
